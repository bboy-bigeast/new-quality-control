...
# 胶粘剂产品专用API端点
def get_adhesive_chart_data(request):
    """获取胶粘剂产品图表数据的API"""
    if request.method == 'GET':
        try:
            # 获取筛选参数
            ...
            if production_line:
                filters['production_line'] = production_line
            if start_date:
                filters['physical_test_date__gte'] = start_date
            if end_date:
                filters['physical_test_date__lte'] = end_date
            
            # 查询数据
            queryset = AdhesiveProduct.objects.filter(**filters).order_by('physical_test_date', 'batch_number')
            ...
            for product in queryset:
                # 使用批次号前8位作为日期（格式：YYYYMMDD）
                batch_date = product.batch_number[:8] if len(product.batch_number) >= 8 else product.physical_test_date.strftime('%Y%m%d')
                labels.append(batch_date)
                ...
                if test_item == 'solid_content':
                    ...
                    data.append(product.color)
                elif test_item == 'initial_tack':
                    data.append(product.initial_tack)
                elif test_item == 'peel_strength':
                    data.append(product.peel_strength)
                elif test_item == 'high_temperature_holding':
                    data.append(product.high_temperature_holding)
                elif test_item == 'room_temperature_holding':
                    data.append(product.room_temperature_holding)
                elif test_item == 'constant_load_peel':
                    data.append(product.constant_load_peel)
                else:
                    data.append(None)
...
def search_adhesive_products(request):
    """胶粘剂产品查询功能"""
    if request.method == 'GET':
        try:
            # 获取查询参数
            ...
            # 查询数据
            products = AdhesiveProduct.objects.filter(**filters).order_by('-physical_test_date', 'batch_number')
            ...
            for product in products:
                product_list.append({
                    ...
                    'physical_test_date': product.physical_test_date.strftime('%Y-%m-%d'),
                    'tape_test_date': product.tape_test_date.strftime('%Y-%m-%d'),
                    'physical_judgment': product.physical_judgment,
                    'tape_judgment': product.tape_judgment,
                    'final_judgment': product.final_judgment,
...
def get_adhesive_moving_range_data(request):
    """获取胶粘剂产品移动极差图数据"""
    if request.method == 'GET':
        try:
            # 获取筛选参数
            ...
            for product in queryset:
                ...
                if test_item == 'solid_content':
                    ...
                    current_value = product.initial_tack
                elif test_item == 'peel_strength':
                    current_value = product.peel_strength
                elif test_item == 'high_temperature_holding':
                    current_value = product.high_temperature_holding
                elif test_item == 'room_temperature_holding':
                    current_value = product.room_temperature_holding
                elif test_item == 'constant_load_peel':
                    current_value = product.constant_load_peel
...
def get_adhesive_capability_analysis_data(request):
    """获取胶粘剂产品能力分析正态分布图数据"""
    if request.method == 'GET':
        try:
            # 获取筛选参数
            ...
            for product in queryset:
                if test_item == 'solid_content':
                    ...
                    value = product.initial_tack
                elif test_item == 'peel_strength':
                    value = product.peel_strength
                elif test_item == 'high_temperature_holding':
                    value = product.high_temperature_holding
                elif test_item == 'room_temperature_holding':
                    value = product.room_temperature_holding
                elif test_item == 'constant_load_peel':
                    value = product.constant_load_peel
                else:
                    value = None
        ...
        try:
            ...
            for product in products:
                product_list.append({
                    ...
                    'external_final_judgment': product.external_final_judgment,
                    'internal_final_judgment': product.internal_final_judgment,
                    'judgment_status': product.judgment_status,
                    'solid_content': product.solid_content,
                    'viscosity': product.viscosity,
                    'acid_value': product.acid_value,
